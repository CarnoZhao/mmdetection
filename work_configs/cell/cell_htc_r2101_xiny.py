...
data = dict(
    ...
    train=[dict(
            ...
            downsample=2.17,
            ann_file=data_root + f'train_xiny/annotations/fold_{fold}.json',
            img_prefix=data_root + 'train_xiny/images/',
            pipeline=train_pipeline) for fold in range(5) if fold != holdout],
    val=dict(
            classes=classes,
            type=dataset_type,
            ann_file=data_root + f'train_xiny/annotations/fold_{holdout}.json',
            img_prefix=data_root + 'train_xiny/images/',
...
nx = 1
work_dir = f'./work_dirs/cell/htcr2101_{nx}x_xiny_d2_800_f{holdout}'
evaluation = dict(
    classwise=True, 
    interval=12, 
    metric=['bbox', 'segm'],
