...
train_pipeline = [
    ...
    dict(type='Resize', img_scale=[(600, 600), (1024, 1024)], keep_ratio=True),
...
data = dict(
    ...
    train=dict(
            type=dataset_type,
            ann_file=data_root + f'train/annotations.json',
            img_prefix=data_root + 'train/images/',
            pipeline=train_pipeline),
    val=dict(
            type=dataset_type,
            ann_file=data_root + f'val/annotations.json',
...
nx = 1
work_dir = f'./work_dirs/food/convx_t_{nx}x_f{holdout}'
evaluation = dict(
    classwise=True, 
    interval=1, 
    metric=['bbox', 'segm'],
